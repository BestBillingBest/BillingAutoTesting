...
@tree
@ExportScenarios
Функционал: Пометка на удаление документ Начисление

Как <Роль>
Я хочу чтобы начисления выполнялись без ошибок
Чтобы <бизнес-эффект>

Сценарий: Я устанавливаю пометку на удаление начислений МКД по территориальному органу 'яТерриториальныйОрган', МКД 'яМКД', с 'ПериодНачало' по 'ПериодОкончание'
    И В командном интерфейсе я выбираю 'Расчет начислений' 'Начисление'
    Тогда открылось окно 'Начисление ФЛ'
