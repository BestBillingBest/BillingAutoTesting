...
Сценарий: Акт замены прибора учета
    ...
    И в поле 'Дата' я ввожу текст '13.06.2024 13:44:31'
    ...
    И в таблице "Список" я перехожу к строке:
        ...
        | Да                   | Россети  | Договор № 000000005 от 11.06.2024 | 000000005     | ОАО 'СК'  | Чайковский | В расчетах | Доходный     |
    И в таблице "Список" я выбираю текущую строку
    Тогда открылось окно 'Акт ввода/вывода/замены прибора учёта ЮЛ (создание) *'
    ...
    Тогда открылось окно 'Объекты расчета'
    И в таблице "Список" я перехожу к строке: 
        | Наименование | Код | Адрес объекта |
        | ОАО ск        | 000000012             | Чайковский г, 20 (ул Красная) гск                |
    И в таблице "Список" я выбираю текущую строку
    Тогда открылось окно 'Акт ввода/вывода/замены прибора учёта ЮЛ (создание) *'
    ...
    И в таблице "ТочкиУчета" я перехожу к строке:
        | N | Точка учета | Услуга | Средства измерения вывод          | Средства измерения         |
        | 1 | 3-ВРУ_Электроэнергия     | Электроэнергия                | Заполнено | Не заполнено |
    ...
    И в таблице "Список" я перехожу к строке:
        ...
        | Акт замены прибора учета | Договор № 000000005 от 11.06.2024 | Россети  | ОАО ск           | ОАО 'СК'  | Автотест   | Чайковский |
